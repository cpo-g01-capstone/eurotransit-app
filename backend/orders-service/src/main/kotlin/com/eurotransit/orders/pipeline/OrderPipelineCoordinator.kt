...
class OrderPipelineCoordinator(
    ...
    suspend fun onOrderPlaced(event: com.eurotransit.orders.events.OrderPlacedEvent) {
        val orderId = event.orderId
        log.info("Pipeline Stage 1: Processing order-placed for {}", orderId)
        
        val order = orderRepository.findById(orderId).awaitSingleOrNull() ?: return
    ...
    suspend fun onInventoryReserved(event: com.eurotransit.orders.events.InventoryReservedEvent) {
        val orderId = event.orderId
        log.info("Pipeline Stage 2: Processing inventory-reserved for {}", orderId)
        
        val order = orderRepository.findById(orderId).awaitSingleOrNull() ?: return
        if (order.status != OrderStatus.RESERVED) return
    ...
    suspend fun onPaymentAuthorized(event: com.eurotransit.orders.events.PaymentAuthorizedEvent) {
        val orderId = event.orderId
