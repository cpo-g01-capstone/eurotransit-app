# ADR-004 — Notifications probes & async lifecycle (readiness reflects lifecycle only)

**Date:** 2026-07-08
**Status:** Accepted (with an open implementation point — see note)
...
## Decision (Option A — lifecycle only)
...
- Binding readiness to downstream would flap and **stall rollouts** on a transient Kafka/DB
  hiccup — the same anti-pattern as a liveness probe checking downstream (a documented
  agent-log trap in `CLAUDE.md`).
- DB-down is already handled by **block-and-lag** (ADR-003) and Kafka reconnect is automatic;
...
- ADR-003 (block-and-lag on DB outage; automatic Kafka reconnect)
